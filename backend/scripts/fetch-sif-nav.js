...
const DEFAULT_START_DATE = '2025-11-01'; // SIFs started in late 2025, pull full history
...
async function main() {
            ...
            scheme.navs.forEach(navEntry => {
              ...
              if (!schemeMap.has(schemeCode)) {
                schemeMap.set(schemeCode, {
                  ...
                  mainCategory: 'Sectoral & Speciality',
                  subCategory: 'SIF',
                  navEntries: []
                });
              }
  ...
  if (totalDataPoints > 0) {
    ...
    const insertScheme = db.prepare('INSERT OR IGNORE INTO schemes (schemeCode, schemeName, isin, schemeCategory, mainCategory, subCategory) VALUES (?, ?, ?, ?, ?, ?)');
    ...
    const insertMany = db.transaction((schemesData) => {
      for (const [schemeCode, data] of schemesData) {
        ...
        if (!exists) {
          insertScheme.run(parsedCode, data.schemeName, data.isin || '', data.schemeCategory || '', data.mainCategory, data.subCategory);
          newSchemes++;
        } else {
          updatedSchemes++;
        }
